document.getElementById("formsalario").addEventListener("submit", function (event) {
    ...
    alert("Seu salário foi salvo com sucesso!")
    exibir_despesas()
})
...
function exibir_despesas() {
    ...
    if (lista_despesas.length === 0) {
        output.innerHTML += "<li>Nenhuma despesa foi cadastrada</li>"
        return
    }
    ...
    resumo.textContent = `TOTAL DE DESPESAS: R$ ${totalDespesas} | SALDO FINAL: R$ ${salario - totalDespesas}`
